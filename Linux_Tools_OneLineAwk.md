...
awk '{while(match($0,/[0-9][0-9][0-9][0-9]+/)) \
{$0=sprintf("%s,%s",substr($0,0,RSTART+RLENGTH-4),substr($0,RSTART+RLENGTH-3))} \
print $0}'
...
awk '{while(match($0,/[^\.0-9][0-9][0-9][0-9][0-9]+/)) \
{$0=sprintf("%s,%s",substr($0,0,RSTART+RLENGTH-4),substr($0,RSTART+RLENGTH-3))} \
print $0}'
...
sed '/pattern/d'                      # 删除含pattern的行。
# 当然pattern可以换成任何有效的正则表达式
...
{print substr(B,l,RSTART-l-f);l=RSTART;sub(/\n/,"",B);f=0};print ""}'
